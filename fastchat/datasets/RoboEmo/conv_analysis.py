...
def getStats(convs):
    ...
    for line in convs:
        ...
        if models is None and "states" in cur_json.keys():
            models = [
                cur_json["states"][0]["model_name"],
                cur_json["states"][1]["model_name"],
            ]
...
if __name__ == "__main__":
    ...
    ), "Please input a json file path with option --input-json"
    ...
    with open(args.input_json, "r") as r:
        ...
        for index, item in enumerate(res):
            ...
            print(
                f'No {index+1}: left_cnt={temp["left_cnt"]}\
                    right_cnt={temp["right_cnt"]} \
                    tie_cnt={temp["tie_cnt"]} \
                    both_bad_cnt={temp["both_bad_cnt"]} '
            )
            print(
                f'{models[0]} percentage:{((temp["left_cnt"]+temp["tie_cnt"])/summ)*100}%, {models[1]} percentage:{((temp["right_cnt"]+temp["tie_cnt"])/summ)*100}%'
            )
